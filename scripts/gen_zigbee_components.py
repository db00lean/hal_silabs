...
class ComponentIndex:
    ...
    def resolve(self, name: str) -> Component | None:
        return self.by_id.get(name) or self.by_name.get(name) or self.by_catalog.get(name)
...
def load_components(zigbee_root: Path) -> ComponentIndex:
    ...
    for scope, relative_dir in ZIGBEE_COMPONENT_DIRS:
        for slcc_path in sorted((zigbee_root / relative_dir).glob("*.slcc")):
...
def condition_is_true(token: str, active_catalogs: set[str], active_component_ids: set[str], index: ComponentIndex) -> bool:
    component = index.resolve(token)
    if component is not None:
        return component.component_id in active_component_ids
...
def should_track_requirement(requirement_name: str) -> bool:
    return requirement_name.startswith("zigbee_")


def should_include_helper_component(component: Component, active_catalogs: set[str]) -> bool:
    if component.component_id in SKIP_HELPER_COMPONENT_IDS:
        return False
    if component.component_id.endswith("_library_internal"):
        return False
    if component.catalog_values:
        return any(value.upper() in active_catalogs for value in component.catalog_values)
    return True
    ...
    while changed:
        ...
        for component_id in current_ids:
            ...
            for requirement in component.requires:
                if not should_track_requirement(requirement["name"]):
                    continue
                ...
                if any(
                    ...
                    continue
                target = index.resolve(requirement["name"])
                if (
                    target is not None
                    and target.component_id not in active_ids
                    and should_include_helper_component(target, active_catalogs)
                ):
                    active_ids.add(target.component_id)
                    changed = True

    return sorted((index.by_id[component_id] for component_id in active_ids), key=lambda item: item.component_id)
...
def generate_component_cmake(
    ...
    component_map: dict[str, Component],
    module_variable: str = "${ZEPHYR_HAL_SILABS_MODULE_DIR}",
...
def generate_report(
    ...
    lines = [
        ...
        "",
        "Generated from the active `sl_component_catalog.h` for the regenerated Z3 Light application.",
        "",
...
def main() -> None:
    ...
    parser.add_argument("--active-catalog-header", type=Path, required=True)
    ...
    cmake = generate_component_cmake(source_manifest, include_dirs, component_map)
    ...
    generated_root = overlay_root / "simplicity_sdk/protocol/zigbee"
    generated_config_root = generated_root / "config"
    report_out = args.report_out.resolve() if args.report_out is not None else generated_root / "Z3_LIGHT_COMPONENTS.md"

    write_if_changed(generated_root / "Kconfig.components", kconfig)
    write_if_changed(generated_root / "components.cmake", cmake)
    write_if_changed(generated_config_root / "sl_catalog_kconfig_translation.h", translation_header)
    write_if_changed(generated_config_root / "sl_component_catalog.h", wrapper_header)
